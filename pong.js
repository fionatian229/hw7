...
var gameoversound, pointsound, osc, paddlesound;
var borders = {
  leftx: 0,
  rightx: 0,
  topy: 0,
...
function setup() {
  ...
  //ball.y = random(110, 300);
  ball.y = random(100, 300); // randomize ball height
  paddlesound = loadSound("Bell.mp3");
  pointsound = loadSound("record.mp3");
  gameoversound = loadSound("Gameover.mp3");
...
function drawStuff() {
  ...
  push();
  fill(255, 173, 5);
  rect(paddle1.x, paddle1.y, paddle1.width, paddle1.length);
  rect(paddle2.x, paddle2.y, paddle2.width, paddle2.length);
  pop();
...
function showResult() {
  if (score.player1 >= 5) {
    textAlign(CENTER);
    text("Player 1 Win!", width / 2, 50);
  }

  if (score.player2 >= 5) {
    textAlign(CENTER);
    text("Player 2 Win!", width / 2, 50);
  }
}
